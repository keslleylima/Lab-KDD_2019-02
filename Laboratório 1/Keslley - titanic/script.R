...
# visualizando a distribuicao dos passageiros pela idade
...
# Encontrando regras de associacao com configura??o padr?o da fun??o
...
# Complete o comando abaixo para ordenar a regra por confianca, e inspecione novamente o resultado
rules.sorted <- sort(COMPLETE)
inspect(rules.sorted)
...
# - = 1, a ocorrencia do valor a esquerda n??o afeta a ocorr??ncia do valor a direita (estatisticamente independentes).
...
# Interpretando regras de sobreviv??ncia de criancas
...
#
# explorando recursos de visualiza????o
#
library(arulesViz)
#geracao de regras 
rules.all <- apriori(titanic.raw, control = list(verbose=F),
...
subrules<-rules.pruned[quality(rules.pruned)$confidence>0.4]
inspect(subrules)
